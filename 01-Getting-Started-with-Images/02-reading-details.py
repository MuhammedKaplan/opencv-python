...
img = cv2.imread("Images\\starry_night.jpg", cv2.IMREAD_UNCHANGED) # Read image with alpha channel
...
img = cv2.imread("Images\\starry_night.jpg", cv2.IMREAD_GRAYSCALE) # Read image to grayscale
cv2.imshow("Image", img) # Show image
cv2.waitKey(0) # Wait for any key press to close window 
